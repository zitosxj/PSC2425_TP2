...
// Obtém um elemento JSON do endereço http
json_t *http_get_json(const char *url) {
    curl_global_init(CURL_GLOBAL_DEFAULT);
    CURL *hcurl = curl_easy_init();
    if (hcurl == NULL) {
        fprintf(stderr, "Erro na função curl_easy_init function\n");
        goto error;
    }
...
// Envia um elemento JSON para endereço http
